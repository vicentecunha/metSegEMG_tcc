...
\begin{figure}[!htb]
	...
	\begin{center}
		\begin{tikzpicture}[node distance = 2cm, auto]
			...
			\node [decision, below = 1cm of init, text width = 8em] (final_detect) {$w_0 \gets w_0 + 1$ \\ \vspace{\onelineskip} $w_0 == (L - W)?$};
			\node [block, left = 1cm of init] (stop) {Segmentos formados pelos BEPs e EEPs identificados \\ \vspace{\onelineskip} FIM};
			\node [decision, below left = 1cm and 3cm of final_detect, text width=12em] (BEP_decide) {$\beta = \frac{1}{W}\sum_{i=w_0+1}^{W}(x_i - x_{i-1})$ \\ \vspace{\onelineskip} $\beta > B$?};
			\node [block, below = 1cm of BEP_decide] (BEP_detected) {$w_0$ é BEP};
			\node [decision, below = 2cm of BEP_detected, text width = 8em] (w0pp) {$w_0 \gets w_0 + 1$ \\ \vspace{\onelineskip} $w_0 == (L - W)?$};
			\node [decision, above right = 1cm and 3cm of w0pp, text width=12em] (EEP_decide) {$\gamma = \sum_{i=w_0+1}^{W}(x_i - x_{i-1})$ \\ \vspace{\onelineskip} $\gamma < C$?};
			...
			\path [line] (final_detect) -| node [near start] {NÃO} (BEP_decide);
			...
			\path [line] (w0pp) -| node [near start] {NÃO} (EEP_decide);
