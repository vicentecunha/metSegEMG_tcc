...
\begin{figure}[H]
	...
	\begin{center}
		...
		\node [test, fill=lcfree!25, text width = 10em, xshift=1em] (C) {Máximo do sinal contido na janela $< T$ e \\ $w_0+W - BEP < l_{mín}$?};
		...
		\node [coord, below=of c1, yshift = -5em] (c4) {};
		...
		\path (lMax.south) to node [near start, xshift=2em] {$não$} (C);
			\draw [o->,lcnorm] (lMax.south) -- (C.north);
		% -------------------------------------------------
		\end{tikzpicture}
	\end{center}
\end{figure}
