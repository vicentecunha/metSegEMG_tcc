...
Utilizando valores de RMS, variância e frequência mediana dos sinais segmentados, uma rede neural artificial será treinada para classificar entre 17 movimentos . O objetivo final deste estudo é avaliar a influência dos métodos de segmentação nas taxas de acerto de classiicação.
...
%-------------------------------------------------------------------------
Os preditores (``entradas'') utilizados pela RNA são o valor \emph{rms}, a variância e a frequência mediana dos segmentos de sinal obtidos pelos métodos MTD1 - MTD4. Tais parâmetros foram selecionados de forma a seguir trabalhos realizados por Favieiro em (Favieiro, ). O valor \emph{rms} ou \emph{root mean square} de um sinal discreto \emph{x} de comprimento $L$ (em Matlab, função \emph{rms()}) é dada pela equação \ref{eq:rms} e a variância deste sinal (em Matlab, função \emph{var()}) é dada pela equação \ref{eq:var}.
...
\begin{equation}
	\label{eq:var}
	var(x) = \sum\limits_{i=1}^{L}(x_i - \mu)^2
\end{equation}
